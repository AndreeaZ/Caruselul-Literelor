...
angular.module('myApp', []).controller('letters', function ($scope, $timeout) {
    ...
    $scope.startWithRandom9Letters = function () {
        ...
        // var id = setTimeout($scope.checkWords, 100);
        // angular.element('#verify22').trigger('click');
        ...
        $scope.updateTimer = function () {
            ...
            $scope.$apply();
            // $timeout($scope.updateTimer, 1000);
        }
    ...
    $scope.checkWords = function () {
        $scope.score = 0;
        $scope.nrWords = 0;
        $scope.words = $scope.unique($scope.inputString.toLowerCase().split(', '));
        $scope.tableVisible = false;
        $scope.inputVisible = false;
